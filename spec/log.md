...
* **2026-09-20** — PLAT-902: deleted `tests/fixtures/module-manifest.schema.json`, the pinned copy of `spec-artifacts-iso`'s module-manifest schema, and the two tests over it (TC-001/TC-033, TC-034). The 2026-08-18 entry above claims this module shipped the FR-035 gate with the module rather than retrofitting it; what shipped judged the manifest against a local copy of `spec-artifacts-iso`'s 24-property fork of `filament-core-service`'s 16-property schema, which keeps the original's `$id`. TC-034 policed that pin; it is red in a local checkout only because the virtualenv holds `spec-artifacts-iso` 0.13.0 while `poetry.lock` pins `^0.18.0`. The copy is deleted because it is a copy. FR-001-AC-5, FR-003-AC-7 and FR-003-CON-3 are retired; conformance is observed at Quire's loader (FR-003-AC-4, AC-6), `quoin module install` (IT-001) and activation. `spec-artifacts-iso` stays a dev dependency for the `edge_types` vocabulary it owns (TC-007, TC-011).
